...
private:
    void run();
    void process_frame(protocol::Frame&& frame);
...
```cpp
void ReadThread::process_frame(protocol::Frame&& frame) {
    // 更新统计
    ...
    if (frame.msgid == kMsgDataSample) {
        ...
        sample.raw_data = std::move(frame.payload);
        
        sample_q_->push(std::move(sample));
        
    } else {
        // 控制帧 (RSP, CFG_REPORT等) -> ResponseQueue
        // 主线程通过ResponseQueue获取并处理这些帧
        response_q_->push(std::move(frame));
    }
}
```
...
bool PowerMonitorSession::send_command_with_retry(
    ...
    for (uint8_t retry = 0; retry <= kMaxRetries; ++retry) {
        try {
            ...
            // 等待响应 (从ResponseQueue读取)
            protocol::Frame rsp;
            // wait_for_specific_response 实现：循环pop_wait直到找到匹配SEQ的RSP或超时
            if (wait_for_response_from_queue(cmd_seq_ - 1, &rsp, kCmdTimeoutUs)) {
