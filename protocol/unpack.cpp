...
EngineeringSample to_engineering(uint32_t vbus_raw, int32_t vshunt_raw, int32_t current_raw,
    ...
    sample.power_w = static_cast<double>(power_raw) * power_lsb;
    sample.energy_j = static_cast<double>(energy_raw) * energy_lsb;
    sample.charge_c = static_cast<double>(charge_raw) * charge_lsb;
    return sample;
}
