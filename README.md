...
It loads the JSON generated by `powermonitor` and renders timeline tracks with logic-analyzer-style interactions:

- Zoom timeline with mouse wheel
- Pan timeline by dragging
- Hover to inspect point values
- Render legacy Pico JSON or multi-source JSON
- For multi-source JSON, show synchronized `pico` and `onboard_cpp` tracks in stacked panels
