...
struct SamplerContext {
    ...
    // Decimation state for temperature reading
...
static void sampler_do_work(SamplerContext* ctx) {
    ...
    // Decimate temperature reading (every 10th sample)
    ...
    // Update counter modulo 10
    ctx->dietemp_skip_counter++;
    if (ctx->dietemp_skip_counter >= 10) {
        ctx->dietemp_skip_counter = 0;
    }
