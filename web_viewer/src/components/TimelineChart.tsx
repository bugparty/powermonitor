...
import { findStartIndex, findEndIndex, findNearestPoint } from "../domain/search";
...
export default function TimelineChart({
    ...
    const visiblePoints = useMemo(() => {
        // Optimization: Use binary search to find range indices (O(log N)) instead of filtering (O(N))
        if (points.length === 0) return [];
        const startIndex = findStartIndex(points, range.start);
        const endIndex = findEndIndex(points, range.end);
    ...
    function handleHoverMove(localX: number | null): void {
        ...
        // Optimization: Use binary search to find nearest point (O(log k)) instead of reduce (O(k))
        const nearestPoint = findNearestPoint(visiblePoints, timeUs);
